...
namespace NgCore {
	public class RenStartup {
		...
		public RenStartup( IHostingEnvironment env ) {
		...
		public void ConfigureServices( IServiceCollection services ) {
			...
			// Unnecessary databaee configuration
			...
			services.AddSwaggerGen( c => {
				c.SwaggerDoc( "v1", new Info { Title = "Angular Universal & ASP.NET Core", Version = "v1" } );
			} ); */
		}
		
		// This method gets called by the runtime. Use this method to configure the HTTP request pipeline.
		public void Configure( IApplicationBuilder app, IHostingEnvironment env, ILoggerFactory log /* SpaDbContext context */ ) {
			log.AddConsole( Configuration.GetSection( "Logging" ) );
			log.AddDebug( );
			// app.UseStaticFiles();
			// Static file settngs for caching of some sort?
			...
			if ( env.IsDevelopment( ) ) {
				...
				// Not using Swagger, so ignore all of this
				/* app.UseSwagger( );
				app.UseSwaggerUI( c => {
					c.SwaggerEndpoint( "/swagger/v1/swagger.json", "My API V1" );
				} );
				// Enable middleware to serve swagger-ui (HTML, JS, CSS etc.), specifying the Swagger JSON endpoint.
				app.MapWhen( x => !x.Request.Path.Value.StartsWith(
					"/swagger",
				...
				app.UseMvc( routes => {
					...
					// Unsure what the sitemap business is all about
